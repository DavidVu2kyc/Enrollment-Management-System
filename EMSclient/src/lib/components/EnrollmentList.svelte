...
<style>
  .page {
    ...
    background-image:
      radial-gradient(ellipse 70% 50% at 10% -5%, rgba(14, 60, 120, 0.45) 0%, transparent 60%),
      radial-gradient(ellipse 50% 40% at 90% 100%, rgba(8, 40, 90, 0.35) 0%, transparent 60%);
  ...
  .stat-card {
    background: linear-gradient(145deg, rgba(12, 28, 55, 0.9) 0%, rgba(7, 18, 38, 0.95) 100%);
  ...
  .stat-num.confirmed { color: #4ade80; }
  .stat-num.pending   { color: #fbbf24; }

  /* ── Controls ────────────────────────────────────── */
  .controls {
    background: linear-gradient(145deg, rgba(12, 28, 55, 0.9), rgba(7, 18, 38, 0.95));
    border: 1px solid rgba(255,255,255,0.07);
    border-radius: 20px;
  ...
  .search-input::placeholder { color: rgba(120, 150, 200, 0.3); }
  ...
  .filter-pill:hover { color: rgba(200, 220, 255, 0.8); }
  ...
  /* ── Empty state ─────────────────────────────────── */
  .empty-state {
    background: linear-gradient(145deg, rgba(12, 28, 55, 0.7), rgba(7, 18, 38, 0.8));
  ...
  <div class="content">
    ...
    <div class="header">
      <div>
        ...
        <p class="header-sub">
          Manage your active enrollments, resolve schedule conflicts, and plan your current semester.
        </p>
      </div>
    ...
    {:else if filterStore.searchedAndFiltered.length > 0}
      <div class="records-grid">
        {#each filterStore.searchedAndFiltered as enrollment (enrollment.id)}
          <EnrollmentRecord
            {enrollment}
    ...
    {:else}
      <div class="empty-state">
        <div class="empty-icon-wrap">
          <svg width="32" height="32" fill="none" stroke="rgba(80,130,220,0.5)" stroke-width="1.8" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
          </svg>
        </div>
        <h3 class="empty-title">No Matches Found</h3>
        <p class="empty-sub">No courses match your current filters or search query.</p>
        <button
          class="reset-btn"
          onclick={() => { search = ""; filterStore.setType("all"); }}
        >
          Reset Filters
        </button>
      </div>
