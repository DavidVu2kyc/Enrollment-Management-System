<script lang="ts">
  ...
  let isFetchingSections = $state(true);
  let availableSections = $state<Section[]>([]);
  ...
  const handleEnroll = async (formData: any) => {
    ...
    try {
      ...
      const result = await response.json();
      enrollmentsStore.add(result.data);
...
<style>
  ...
  .spinner-bg {
    ...
    border: 3px solid rgba(30, 77, 183, 0.12);
    border-radius: 50%;
  }
  ...
  /* ── Error state ─────────────────────────────── */
  .error-card {
    background: linear-gradient(135deg, rgba(180,28,28,0.13), rgba(120,10,10,0.18));
    ...
    text-align: center;
    gap: 1rem;
    box-shadow: 0 16px 48px rgba(180,28,28,0.15), inset 0 1px 0 rgba(255,255,255,0.04);
    animation: fadeUp 0.5s 0.15s cubic-bezier(0.22,1,0.36,1) both;
  }
  ...
  <div class="content">
    ...
    <div class="header">
      ...
      <div>
        ...
        <p class="header-sub">
          Initiate a course enrollment for the upcoming academic cycle. Slot authorization is processed in real time.
        </p>
      </div>
    </div>
    ...
    {#if isFetchingSections}
      <div class="loading-card">
        ...
        <p class="loading-label">Synchronizing Course Catalog</p>
      </div>

    {:else if error}
      <div class="error-card">
        <div class="error-icon-wrap">
          <svg width="22" height="22" fill="none" stroke="#fca5a5" stroke-width="2" viewBox="0 0 24 24">
    ...
    {:else}
      <div class="form-wrap">
        ...
        {/if}
        <EnrollmentForm {availableSections} onSubmit={handleEnroll} {isLoading} />
      </div>
    {/if}
