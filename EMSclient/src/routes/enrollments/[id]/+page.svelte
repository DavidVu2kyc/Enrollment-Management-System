...
<script lang="ts">
  ...
  // Using SSR data instead of fetching on mount
  ...
  const handleUpdate = async (formData: any) => {
    ...
    submitError = null;
    try {
      // Create FormData from the submitted json to submit to the ?/default action
      const body = new FormData();
      if (formData.sectionId) body.append("sectionId", formData.sectionId);
      ...
      // result.data is typically the JSON representation of the ActionData
      const actionResult = result.data ? JSON.parse(result.data) : null;
      if (actionResult && !actionResult[0]?.success) {
        throw new Error(
          actionResult[0]?.error || "Failed to update enrollment",
        );
      }
...
<svelte:head>
  <title>Edit Registration — EMS</title>
  ...
  <div class="content">
    ...
    <div class="header">
      ...
      <div>
        ...
        <p class="header-sub">
          Modify an existing course enrollment. Changes will be reflected
    ...
    {#if isFetching}
      <div class="loading-card">
        ...
        <p class="loading-label">Retrieving Enrollment Record</p>
      </div>
    {:else if error}
      <div class="error-card">
        ...
        <p class="error-title">Record Unavailable</p>
...
<style>
  ...
  .spinner-bg {
    position: absolute;
    inset: 0;
    border: 3px solid rgba(26, 107, 74, 0.15);
    border-radius: 50%;
  }
