...
<script lang="ts">
  ...
  // ─────────────────────────────────────────────────────────────
  const handleSectionGet = async (sectionId: number) => {   // ✅ was empty / missing
    if (!sectionId) { selectedSection = null; return; }

    sectionFetching = true;
    sectionFetchErr = null;

    try {
      selectedSection = await apiClient.get<SectionResponse>(`/sections/${sectionId}`);
    } catch (err: any) {
  ...
  // ─────────────────────────────────────────────────────────────
  const handleUpdate = async (formData: { sectionId?: number; status?: string }) => {
    isLoading   = true;
    submitError = null;
    ...
    try {
      ...
      if (formData.sectionId) body.append("sectionId", String(formData.sectionId));
      if (formData.status)    body.append("status",    formData.status);

      const response             = await fetch("?/default", { method: "POST", body });
      const result: ActionResult = deserialize(await response.text()); // ✅ not .json()

      if (result.type === "failure") {
        submitError = (result.data as any)?.message ?? "Failed to update enrollment";
        return;
      }
      ...
      <div class="section-panel" class:section-panel--full={seats <= 0}>
        ...
        <div class="sp-chips">
          ...
          {#if selectedSection.room}
            <span class="sp-chip">📍 {selectedSection.room.building} {selectedSection.room.roomNumber}</span>
          {/if}
          <span class="sp-chip">🪑 {selectedSection.enrolledCount} / {selectedSection.maxSeats}</span>
        </div>
        {#if seats <= 0}
    ...
    <div class="form-wrap">
      {#if submitError}
        <div class="submit-error" role="alert">
          ...
          <p>{submitError}</p>
          <button class="dismiss-btn" onclick={() => (submitError = null)}
...
<style>
  ...
  .header-icon {
    ...
    box-shadow: 0 8px 28px rgba(26,107,74,0.4), inset 0 1px 0 rgba(255,255,255,0.15);
  ...
  .eyebrow {
    ...
    text-transform: uppercase; color: rgba(52,199,130,0.6); margin-bottom: 0.5rem;
  }

  h1 {
    font-family: "DM Serif Display", serif; font-style: italic;
    font-size: clamp(1.8rem, 4vw, 2.75rem); font-weight: 400;
    color: #fff; line-height: 1.05; letter-spacing: -0.02em; margin: 0 0 0.6rem;
  }

  .header-sub { font-size: 0.84rem; color: rgba(140,170,210,0.55); line-height: 1.6; margin: 0; max-width: 480px; }

  .id-badge {
    display: inline-flex; align-items: center; gap: 0.45rem; margin-top: 0.75rem;
    background: rgba(52,199,130,0.08); border: 1px solid rgba(52,199,130,0.18);
    border-radius: 8px; padding: 0.3rem 0.75rem;
    font-size: 0.7rem; font-weight: 600; letter-spacing: 0.1em;
    text-transform: uppercase; color: rgba(52,199,130,0.7);
  }

  .id-badge-dot { width: 5px; height: 5px; background: rgba(52,199,130,0.6); border-radius: 50%; }

  /* ── Section status / preview ─────────── */
  .section-status {
    display: flex; align-items: center; gap: 0.75rem;
    background: rgba(59,111,212,0.05); border: 1px solid rgba(59,111,212,0.15);
    border-radius: 14px; padding: 0.9rem 1.1rem; margin-bottom: 1.25rem;
    font-size: 0.78rem; font-weight: 500; color: rgba(140,170,210,0.5);
  ...
  .section-panel {
    background: rgba(59,111,212,0.05); border: 1px solid rgba(59,111,212,0.18);
    border-radius: 18px; margin-bottom: 1.5rem; overflow: hidden;
    animation: fadeUp 0.3s cubic-bezier(0.22,1,0.36,1) both;
  }

  .section-panel--full { border-color: rgba(251,191,36,0.25); background: rgba(251,191,36,0.04); }
  ...
  .sp-codes { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.35rem; }

  .sp-code {
    font-size: 0.6rem; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase;
    color: rgba(100,160,255,0.85); background: rgba(59,111,212,0.15);
    border: 1px solid rgba(59,111,212,0.25); border-radius: 5px; padding: 0.15rem 0.5rem;
  }

  .sp-sec { font-size: 0.6rem; font-weight: 600; color: rgba(140,170,210,0.5); }

  .sp-title {
    font-family: "DM Serif Display", serif; font-style: italic;
    font-size: 1.1rem; color: #fff; margin: 0; line-height: 1.2;
  }

  .sp-seats { text-align: right; flex-shrink: 0; }
  ...
  .seats-lbl { font-size: 0.58rem; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; opacity: 0.5; }

  .seats-ok  .seats-num { color: #34c782; }
  .seats-warn .seats-num { color: #fbbf24; }
  .seats-full .seats-num { color: #f87171; }

  .sp-chips { display: flex; flex-wrap: wrap; gap: 0.5rem; padding: 0.9rem 1.5rem; }

  .sp-chip {
    display: inline-flex; align-items: center; gap: 0.4rem;
    background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07);
    border-radius: 7px; padding: 0.3rem 0.7rem;
    font-size: 0.68rem; color: rgba(160,185,220,0.6);
  }

  .sp-warning {
    margin: 0 1.25rem 1rem; padding: 0.65rem 0.9rem;
    background: rgba(251,191,36,0.08); border: 1px solid rgba(251,191,36,0.2);
    border-radius: 9px; font-size: 0.72rem; font-weight: 600; color: rgba(251,191,36,0.8);
  }

  /* ── Submit error ─────────────────────── */
  .submit-error {
    display: flex; align-items: center; gap: 0.75rem;
    background: rgba(180,28,28,0.1); border: 1px solid rgba(248,113,113,0.18);
    border-radius: 12px; padding: 0.9rem 1.1rem; margin-bottom: 1.25rem;
    animation: shake 0.4s ease, fadeUp 0.3s ease;
  }

  .submit-error p { font-size: 0.82rem; color: #fca5a5; margin: 0; flex: 1; }

  .dismiss-btn {
    background: none; border: none; color: rgba(252,165,165,0.5);
    cursor: pointer; font-size: 0.75rem; padding: 0; transition: color 0.15s;
  }
  .dismiss-btn:hover { color: #fca5a5; }

  .form-wrap { animation: fadeUp 0.5s 0.15s cubic-bezier(0.22,1,0.36,1) both; }
  ...
  @keyframes sheen   { 0%,75%,100% { transform:translateX(-100%); } 40% { transform:translateX(100%); } }
